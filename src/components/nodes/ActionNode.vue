...
const isValid = computed(() => {
  ...
  switch (props.type) {
    ...
    case 'send_whatsapp_api':
      return Boolean(config.meta_template_name)
    case 'send_email':
  ...
  switch (props.type) {
    ...
    case 'send_whatsapp_api':
      if (config.meta_template_name) {
        const paramCount = config.template_params ? Object.keys(config.template_params).length : 0
        return () => h('div', { class: 'summary-content sms-summary' }, [
          h('div', { class: 'sms-template-name' }, config.meta_template_name),
          h('div', { class: 'summary-tag' }, 'API Oficial'),
          paramCount > 0
            ? h('div', { class: 'sms-template-preview' }, `${paramCount} ${paramCount === 1 ? 'parametro' : 'parametros'} mapeados`)
            : null,
        ])
      }
      return () => h('div', { class: 'summary-placeholder' }, 'Clique para configurar')
