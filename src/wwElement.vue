  ...
  <div class="flow-builder-wrapper" :style="wrapperStyle">
    ...
    <div class="flow-builder-container" :style="containerStyle" :class="{ 'read-only': isReadOnly }">
      ...
      >
        ...
        <template #node-send_whatsapp_api="nodeProps">
          <ActionNode
            v-bind="nodeProps"
            type="send_whatsapp_api"
            @add-node="handleOpenAddNodeMenu"
            @delete-node="handleDeleteNode"
          />
        </template>
    ...
    <transition name="panel-slide">
      <NodeConfigPanel
        ...
        :whatsapp-api-templates="whatsappApiTemplates"
...
// WhatsApp API Templates (Meta Cloud API) - from collection or ww-config binding
const whatsappApiTemplates = computed(() => {
  // First try from ww-config binding (props.content.whatsappApiTemplates)
  const bound = props.content?.whatsappApiTemplates
...
const getNodeHeight = (node) => {
  ...
  const baseHeights = {
    ...
    send_whatsapp_api: 160, // ActionNode with WhatsApp API
...
const getNodeWidth = (node) => {
  ...
  const nodeWidths = {
    ...
    send_whatsapp_api: 280, // ActionNode width
...
const getDefaultConfig = (type) => {
  const configs = {
    ...
    send_whatsapp_api: { meta_template_name: '', meta_template_language: 'pt_BR' },
